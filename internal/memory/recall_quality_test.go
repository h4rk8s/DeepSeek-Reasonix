...
func seedRecallQualityCorpus(t testing.TB) (Store, time.Time) {
	...
	memories := []Memory{
		...
		{Name: "truth-locked-old", Title: "Durable port truth", Description: "FRP remote port 10023 belongs to mini SSH", Type: TypeProject, Body: "This old confirmed fact remains true and may rank lower, but must not be discarded.", SourceKind: "user_confirmed"},
	}
	for _, m := range memories {
		savedAt := now
		if m.Name == "truth-locked-old" {
			savedAt = now.AddDate(-3, 0, 0)
		}
		if _, err := store.SaveAt(m, savedAt); err != nil {
			t.Fatal(err)
		}
	}
