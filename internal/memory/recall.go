...
func rerankMemoryHits(hits []memoryHit, limit int, options normalizedRecallOptions) []memoryHit {
	...
	if !options.diversity {
		if options.staleness && len(hits) > 2 {
			// Preserve the strongest lexical hit, but let canonical freshness
...
func memoryStalenessFactor(m Memory, options normalizedRecallOptions) float64 {
	...
	freshness := FreshnessFor(m, options.now)
	...
	reference := freshnessReferenceTime(m)
	if reference.IsZero() || !options.now.After(reference) {
		return 1
	}
	age := options.now.Sub(reference)
	factor := math.Exp2(-float64(age) / float64(options.stalenessHalfLife))
	factor = math.Max(0.5, factor)
	if freshness == FreshnessStale {
		factor = math.Min(0.75, factor)
	}
	return factor
}
...
func formatMemoryHits(query string, hits []memoryHit) string {
	...
	for i, hit := range hits {
		...
		fmt.Fprintf(&b, "\n%d. score=%.3f rank=%.3f diversity=%.3f staleness=%.3f id=%s revision=%d name=%s scope=%s type=%s source=%s/%s title=%s\n   reference: %s\n   description: %s\n   snippet: %s\n",
			i+1, hit.Score, hit.RankScore, hit.DiversityPenalty, hit.StalenessFactor, m.ID, m.Revision, m.Name, NormalizeFactScope(string(m.Scope)), NormalizeType(string(m.Type)), NormalizeFactScope(string(m.Scope)), emptyLabel(m.SourceKind), displayTitle(m.Title, m.Name), providerMemoryReference(m), oneLine(m.Description), hit.Snippet)
	}
...
func formatMemory(_ Store, m Memory) string {
	...
	fmt.Fprintf(&b, "source: %s/%s\n", NormalizeFactScope(string(m.Scope)), emptyLabel(m.SourceKind))
