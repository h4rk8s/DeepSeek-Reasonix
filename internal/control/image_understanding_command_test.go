...
`), 0o755); err != nil {
	...
	got, used, err := c.tryLocalImageUnderstanding(context.Background(), "see @shot.png")
	if err != nil || !used {
		t.Fatalf("tryLocalImageUnderstanding used=%v err=%v", used, err)
	}
...
`), 0o755); err != nil {
	...
	if _, used, err := c.tryLocalImageUnderstanding(context.Background(), "see @shot.png"); err != nil || !used {
		t.Fatalf("tryLocalImageUnderstanding used=%v err=%v", used, err)
	}
