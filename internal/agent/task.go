...
"properties":{
  ...
  "isolation":{"type":"string","enum":["none","worktree"],"description":"Optional filesystem isolation for writer subagents. Defaults to none. ` + string(SubagentIsolationWorktree) + ` creates a repo-local Git worktree resource; merge-back and finalization are separate explicit operations."},
...
func (t *TaskTool) RunProfileSpec(ctx context.Context, spec ProfileExecSpec) (result string, err error) {
	...
	runSession := func(runCtx context.Context, sink event.Sink, writerAlreadyRegistered bool) (string, error) {
		...
		return t.runSubSession(WithSubagentWriteClaim(runCtx, spec.Grant.WritePaths), composeChildTaskPrompt(spec), childWorkspaceRoot, subReg, sink, maxSteps, prov, pricing, ctxWin, run.Session, childDepth, recoveryTaskID, usageModelRef, mutationObserver, childWriteRoots)
	}
...
func (t *TaskTool) resolveSubagentWorkspace(ctx context.Context, requested SubagentIsolation, continueFrom, legacyForkFrom, taskName string) (string, worktree.Resource, error) {
...
func appendSubagentIsolationReference(b *strings.Builder, meta SubagentMeta) {
	...
	b.WriteString("Merge-back is explicit: inspect the exact status and conflict set before merging; finalization is a separate explicit operation.\n")
}
