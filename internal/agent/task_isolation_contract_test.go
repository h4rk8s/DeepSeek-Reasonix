...
func TestTaskToolPersistsWorktreeIsolationMetadata(t *testing.T) {
	...
	for _, want := range []string{"Worktree isolation: iso-test", "Workspace: " + childWorkspace, "Branch: reasonix/subagent-test", "Merge-back is explicit", "finalization is a separate explicit operation"} {
...
func TestTaskToolWriterRunsInsideWorktreeAndContinueReusesResource(t *testing.T) {
	...
	out, err := task.Execute(testTaskContext(), []byte(`{"prompt":"write the isolated file","description":"cache audit","isolation":"worktree"}`))
	if err != nil {
	...
	wantParent := filepath.Join(resources[0].SourceRoot, ".worktree")
	if !cleanPathEqual(filepath.Dir(resources[0].WorktreeRoot), wantParent) || !strings.HasPrefix(filepath.Base(resources[0].WorktreeRoot), "reasonix-cache-audit-") {
		t.Fatalf("isolated worktree = %q, want task-specific child of %q", resources[0].WorktreeRoot, wantParent)
	}
...
func TestTaskToolReadOnlyNoneDoesNotAllocateWorktree(t *testing.T) {
	source := initTaskIsolationRepo(t)
	manager := worktree.NewManager(filepath.Join(t.TempDir(), "managed"))
	parent := tool.NewRegistry()
	parent.Add(subagentRegistryTool{name: "read_file", readOnly: true})
	task := NewTaskTool(&captureSystemProvider{}, nil, parent, 20, 0, 0, 0, 0, 0, 0, 0, "", "sys", nil, 0, "", "", nil).
		WithTranscripts(NewSubagentStore(t.TempDir()), source, "base-model", "base-effort").
		WithWorkspaceIsolation(manager, func(_ context.Context, _ string, _ []string, _, _ int) (*tool.Registry, error) {
			t.Fatal("read-only shared task must not rebuild a worktree registry")
			return nil, nil
		})
	spec, err := task.buildTaskSpec(testTaskContext(), "inspect only", "read only", "", nil, nil, 0, "", "", "", "", "none", false, true)
	if err != nil {
		t.Fatalf("build read-only spec: %v", err)
	}
	if _, err := task.RunProfileSpec(testTaskContext(), spec); err != nil {
		t.Fatalf("run read-only spec: %v", err)
	}
	resources, err := manager.List(context.Background())
	if err != nil {
		t.Fatalf("List: %v", err)
	...
	if len(resources) != 0 {
		t.Fatalf("read-only none allocated resources: %+v", resources)
	}
	if _, err := os.Stat(filepath.Join(source, ".worktree")); !os.IsNotExist(err) {
		t.Fatalf("read-only none created repo-local worktree storage: %v", err)
	}
}
