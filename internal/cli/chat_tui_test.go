...
func TestStatusLineWrapAccounting(t *testing.T) {
	...
	// When running, the working and model-state rows should increase the same
	// footer's reserved height.
	m2 := newInboxTestChatTUI(t)
	m2.seedInbox("feedback")
	m2.width = m.width
	idleCount := m2.computeStatusLineCount(m2.width)
	m2.state = tuiRunning
	m2.elapsed = 5
	m2.turnTokens = 100
...
func TestRunningQueueAndTodoKeepComposerVisible(t *testing.T) {
	...
	m.todos = []event.Todo{
		{Content: "诊断 region 信号", Status: "in_progress"},
