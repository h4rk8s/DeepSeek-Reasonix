...
func TestRunningQueueAndTodoKeepComposerVisible(t *testing.T) {
	ctrl := control.New(control.Options{})
	m := newChatTUI(ctrl, "", make(chan event.Event, 1), 54)
	m.state = tuiRunning
	m.elapsed = 157
	m.turnTokens = 4900
	m.pendingInterject = []string{
...
func TestQueueIndicatorRendering(t *testing.T) {
	...
	m.pendingInterject = []string{"see @.reasonix/attachments/clipboard-20260710-130309.438441-000001.png"}
	m.queueEditCursor = -1
	qi = m.renderQueueIndicator()
