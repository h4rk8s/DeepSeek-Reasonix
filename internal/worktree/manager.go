...
const (
	...
	CleanupStateRetained CleanupState = "retained"
...
func (m *Manager) Create(ctx context.Context, workspaceRoot string, policy CreatePolicy) (Resource, error) {
	...
	if policy.Kind == KindSubagent {
		// The repository-local namespace is part of the isolation contract. Set
		// its repository-local exclusion before evaluating dirtiness so a
		// pre-created .worktree directory does not reject an otherwise clean repo.
	...
	for range 5 {
		...
		_, worktreeRoot, allocationErr := worktreeAllocation(ctx, info, managedRoot, id, policy)
		if allocationErr != nil {
			return Resource{}, allocationErr
		}
		...
		res := Resource{
			...
			SourceBranch:   strings.TrimSpace(info.Branch),
			Branch:         branch,
			...
			CreatedAt:      now,
			metadataPath:   resourceMetadataPath(managedRoot, repoStorageKey(info.commonDir), id),
		}
		if err := writeResourceMergeMetadata(res); err != nil {
			return Resource{}, m.rollbackFailedCreate(ctx, res, err)
		}
...
func (m *Manager) List(ctx context.Context) ([]Resource, error) {
	...
	err = filepath.WalkDir(managedRoot, func(path string, d os.DirEntry, err error) error {
		...
		if res.CleanupState == CleanupStateActive || res.CleanupState == CleanupStateRetained {
			if _, statErr := os.Stat(resourceCheckoutRoot(res)); os.IsNotExist(statErr) {
...
func (m *Manager) Status(ctx context.Context, res Resource) (Status, error) {
	checkoutRoot := resourceCheckoutRoot(res)
	if strings.TrimSpace(checkoutRoot) == "" {
		return Status{}, errors.New("worktree root is required")
	}
	out, stderr, err := runGit(ctx, checkoutRoot, "status", "--porcelain=v1", "--untracked-files=normal")
	if err != nil {
...
func (m *Manager) Diff(ctx context.Context, res Resource) (Diff, error) {
	...
	patch, stderr, err := runGit(ctx, resourceCheckoutRoot(res), "diff", "--binary", "HEAD")
	if err != nil {
...
func (m *Manager) Remove(ctx context.Context, res Resource) (Resource, error) {
	...
	head, err := worktreeHead(ctx, resourceCheckoutRoot(res))
	if err != nil {
		return Resource{}, err
	}
...
func (m *Manager) GC(ctx context.Context) (GCResult, error) {
	...
	for _, res := range resources {
		...
		if err := removeResourceControlMetadata(res); err != nil {
			return result, err
		}
		if err := m.writeResource(res); err != nil {
			return result, err
...
func (m *Manager) rollbackFailedCreate(ctx context.Context, res Resource, cause error) error {
	result := Result{
	...
	if err := RollbackCreate(ctx, result); err != nil {
		return fmt.Errorf("publish managed worktree metadata and roll back allocation: %w", errors.Join(cause, fmt.Errorf("exact-clean rollback failed and the worktree was preserved: %w", err)))
	}
	return cause
}
...
func resourceCheckoutRoot(res Resource) string {
	if res.CleanupState == CleanupStateRetained && strings.TrimSpace(res.RecoveryRoot) != "" {
		return res.RecoveryRoot
	}
	return res.WorktreeRoot
}
