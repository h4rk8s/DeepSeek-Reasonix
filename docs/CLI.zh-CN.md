...
所有 usage 都有同一币种定价时，结果会输出 `total_cost` 及对应的 ISO
`currency`（DeepSeek 官方定价目前为 `CNY` 或 `USD`）。只有全部价格均为 USD 时，
才输出标准化的 `total_cost_usd` 和精确的 `total_cost_usd_ticks`。
混合原币费用继续通过 `cost_quote` 和 `original_totals` 返回，不会伪造跨币种合计。
