...
本轮固定上游：`eac151f2c04bfe4b6dbec86401652c4a74b61731`（`desktop-v1.33.0-43-geac151f2c`；最新发布 tag 为 `desktop-v1.33.0`）
本轮验证候选：以 `git log -1` 为准。
当前 patch stack：固定为 12 个语义 patch；不再把已经被上游覆盖的基础 TUI、图片和 build metadata 能力作为独立 patch 维护。
...
`merge-base..HEAD`，因此会重放长期线上的**完整 12 patch 语义栈**，包括 worktree、usage、memory
等核心协议补丁，而不是按旧数量或旧 SHA 选择提交。每次同步仍需用 `range-diff` 核对补丁映射，
不能只看 rebase 成功。
...
| `cmd/reasonix/main.go`, `Makefile`, `internal/cli/buildinfo.go` | build metadata | 已由上游覆盖；本地不再维护，出现 diff 应先按回归检查 |
...
### 2026-08-29 v1.33.0 跟进

- 固定上游从 `7775342453b24e28efbff25061dc5f59398f126b` 推进到
  `eac151f2c04bfe4b6dbec86401652c4a74b61731`，吸收 80 个提交。主要收益包括：v1.32 的 durable ordered turns、
  官方 MCP SDK 和远程会话可靠性；v1.32.1 的自然完成语义；v1.33.0 的 transcript/session 修复；以及 tag 之后的
  MCP Apps、多轮 reasoning timeline、composer 自动高度和 transcript reader transaction。
- 12 个本地语义 patch 全部保留原顺序，无 drop、squash 或重排。`range-diff` 中 8 个内容等价，4 个按上游结构
  做语义适配：terminal title、usage ledger、interactive disclosures 和 sync workflow。未发现已经被上游完整覆盖、
  可以安全退休的本地 patch。
- replay 有两处冲突。`topic-activation.test.tsx` 保留上游 `controller.state.hydrating === false` 等待条件，避免本地
  transcript patch 退回旧 hydration 语义；`remote-project-tree.test.tsx` 保留上游
  `remotePendingSelectionSource` 命名与调用，避免维护 patch 覆盖新的 remote pending-selection 边界。两处都只在
  上游新契约上恢复本地测试意图，没有采用整侧 `ours/theirs`。
- 全量前端门禁暴露两处纯上游测试滞后，本地在 sync workflow patch 内做最小兼容：
  `transcript-scroll-writer.test.ts` 不再使用超出项目 ES2021 lib 的 `Array.prototype.at`；
  `composer-run-strip.test.tsx` 按新实现读取隐藏的 `.composer__input--measure` 测量 textarea，而不是给可见输入框伪造
  `scrollHeight`。这两条都不修改运行时代码，上游修复对应测试后应直接删除。
- 已通过 TUI interaction regression、冲突相关 Go 包、`go vet ./...`、根模块全量 Go 测试、Desktop Go 全量、
  Node 24 前端依赖锁定安装、frontend build 和完整 `test:all`（含 history performance benchmark）。React `act(...)`
  警告、Vite 大 chunk/动态 import 提示和 macOS linker 重复 `-lobjc` 仍是非阻断上游警告。

### 2026-08-28 本地补丁语义重整

- 固定上游仍为 `7775342453b24e28efbff25061dc5f59398f126b`；本轮不 fetch、不引入新的上游代码，只整理本地提交边界。
- 旧长期线 `a191999676c46576360db578ed94751caf378762` 的 17 个提交被重建为下面 12 个语义 patch。
- 重建完成、修改本文档前，新旧 Git tree 都是 `bb878ac535308d1fc378832bb23b69e88d0f9730`；运行时代码和测试文件逐字节一致，没有功能删除或隐式改写。
- `build metadata` 不再是本地 patch：当前上游已经提供本地需要的版本构建信息，旧补丁若再次出现应判定为冗余。
- 基础 thinking 折叠、基础图片粘贴、基础 responsive status 继续由上游承担；本地 patch 只表达点击 disclosure、真实图片来源兼容、两行信息架构等增量语义。

当前 12 patch 的固定顺序：

| # | 语义 patch | 维护边界 |
|---:|---|---|
| 1 | `feat(cli): preserve terminal title controls` | `/title`、OSC title、字段配置和 rename 同步 |
| 2 | `fix(agent): keep planner text out of visible transcript` | planner/executor 单一可见回答与 handoff 恢复 |
| 3 | `feat(vision): add managed image understanding pipeline` | CleanShot/Raycast/path 兼容、sidecar、结构化理解和缓存稳定性 |
| 4 | `feat(cli): preserve interactive transcript architecture` | 本地 transcript/composer/status 信息架构基础 |
| 5 | `feat(subagent): add recoverable worktree isolation lifecycle` | writer 隔离、apply/conflict/GC/resume；默认仍为 `none` |
| 6 | `feat(usage): add fail-closed shared usage ledger` | CLI/ACP usage 归因，未知成本不伪装为零 |
| 7 | `feat(memory): add provenance-aware diverse recall` | metadata、staleness、去重复与旧格式兼容 |
| 8 | `test(tool): benchmark the canonical tool contract` | 单一 canonical executor 的方言基准门禁 |
| 9 | `fix(config): keep runtime loading strict and read-only` | 解析失败显式终止，普通启动不写盘 |
| 10 | `feat(cli): complete interactive transcript disclosures` | Thought/Image 展开、hover、resume parity、视觉锚点和 vision footer |
| 11 | `feat(inbox): add queue clearing and slash completion` | `/queue clear`、补全、附件友好预览和 pause 恢复 |
| 12 | `chore(sync): maintain the local patch stack workflow` | 私有同步脚本、回归脚本、兼容清理和本台账 |

同步时按这 12 个边界判断保留、适配或退休，禁止把旧的 17 个提交 SHA 当作协议真源。
