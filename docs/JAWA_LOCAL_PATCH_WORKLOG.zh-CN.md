...
- 补丁结构：14 个产品语义 owner + 2 个维护闸门 + 1 个临时 renderer pin，共 17 个受维护单元。计费日历、队列图片预算、canonical session 适配、runtime context pressure、resume/compaction progress 和 Ghostty bottom-rail redraw 等 follow-up fix 仍归所属 owner，不增加语义补丁数量。固定上游至当前 HEAD 共 43 个线性提交：原 41 个补丁逐项重放、1 个结构基线与台账维护提交、1 个 CLI 底栏 follow-up；准确提交列表以本文给出的 `git log` 命令为准。
...
- 参数错误复用统一的未执行/可恢复诊断与 shared repeat reminder；不维护私有失败计数器。
...
- legacy MCP envelope；
- one retry；
- repeated failure reminder 与成功后 streak reset；
...
## 2026-09-14 v1.38.7+ 重放判定

- 旧上游：`e2145b031deef603d9ec1acc9d30f9a1e1ac9325`（desktop-v1.38.6）。
- 固定新上游：`744c2e94ec77b577fbe3f1c93c769b1c3388bd21`（desktop-v1.38.7-31-g744c2e94ec77），固定区间 51 个提交。
- 旧长期线：`83046afef608633a1395f613c3c7cca4e50dfa9b`；18 个提交保持原顺序重放，未新增平行产品线。
- 上游新增 Harness chat/permission/Windows sandbox、DSH session ownership/persistence 和 goal lifecycle；本地补丁均在这些 owner 上增量适配，不复制第二套状态机。
- 上游已退休 evidence/recovery hard gate。capability patch 保留 deterministic repair 和第三次重复失败的 advisory reminder，不恢复 hard pause、旧 recovery confirmation UI 或私有 storm breaker。
...
| owner | 冲突面 | 处理 |
|---|---|---|
| vision/config | `internal/config/edit.go` | 同时保留上游 Desktop permission setter 与本地 image setter；二者字段和 owner 独立。 |
| CLI shell | config、typed todo | 接入上游 permission model 和 `[]event.Todo`，保留 title/presentation/status contract。 |
| subagent | boot assembly | 采用上游 parent/session ownership，保留 child-root registry；不复活已退休 review/evidence tool。 |
| usage | event/background payload | 同时保留上游 `UserMessage` 与本地 background lifecycle/usage attribution。 |
| memory | tool registration | 采用上游 workspace-write 与新 recovery model，保留唯一 `memory_recall` reranker。 |
| transcript/Desktop | session-v3 buffer | 以 `transcript.Message` 为共享 owner；删除重复 Desktop buffer，只增强上游 history adapter。 |
| CLI projection | TUI/config/control | 跟随 typed todos、standard delivery floor 和 workspace-write default，保留 disclosure/footer/lazy reasoning。 |
| capability | catalog search、repeat handling | 抽出共享 catalog ranking，保留 snapshot metadata；将旧 hard breaker 测试改为上游 advisory reminder contract。 |
...
- machine session list 不再把尚未完成异步 metadata projection 的 canonical row 当成空会话丢弃。metadata-ready 的真空会话仍由共享 catalog 过滤；短生命周期的 `session list --json` 现在能立即列出已有 v4 历史，而不是每次启动都在 rebuild 完成前返回空数组。
...
## 2026-09-25 Ghostty bottom-rail follow-up

- 新旧 CLI 进程均可在长流式输出时出现排队行残影、composer 消失或 footer 重叠；同一新进程在 Ghostty 宽度轻微改变后立即恢复。模型级连续帧在 48/72/100 列均保持正确行数、宽度、单条队列和输入框，故不改会话/队列数据 owner。
- 在既有 CLI presentation owner 内，将按 viewport 实际滚动请求整屏重绘的保护从 Warp 扩到 macOS Ghostty；稳定帧不重复清屏。运行中 `Ctrl+L` 只请求重绘并保留当前任务、草稿与队列；空闲态保留原清屏行为。
- 此补丁不声称修复 Bubble Tea/Ultraviolet 所有物理帧问题。单元与全仓测试验证了重绘请求和状态保持；真实 Ghostty 新进程复现仍是现场验收门。当前运行进程不会因磁盘二进制更新而热替换。
